...
const harnessModulePromise = import("./harness");

async function main() {
  const harness = await harnessModulePromise;
  await import("./payto.spec.ts");
  await import("./gate.scheduler.spec.ts");
  await import("./privacy.spec.ts");
  await import("./sbr.spec.ts");
  await harness.run();
}
